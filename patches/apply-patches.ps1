...
git submodule deinit -f deps/wil 2>&1 | Out-Null
if (Test-Path -LiteralPath (Join-Path $repoRoot 'deps\wil')) {
  Remove-Item -LiteralPath (Join-Path $repoRoot 'deps\wil') -Recurse -Force -ErrorAction Stop
}
git rm -q --cached deps/wil 2>&1 | Out-Null
...
foreach ($rel in $features) {
  ...
  New-Item -ItemType Directory -Path (Split-Path $dst) -Force -ErrorAction Stop | Out-Null
...
foreach ($patch in $patches) {
  ...
  git apply --whitespace=nowarn --ignore-whitespace $patch.FullName
  if ($LASTEXITCODE -ne 0) {
    Write-Host "  git apply failed, retrying with --3way ..."
    git apply --3way --whitespace=nowarn --ignore-whitespace $patch.FullName
    if ($LASTEXITCODE -ne 0) {
